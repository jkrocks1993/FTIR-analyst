#!/usr/bin/env python3
"""
FTIR Spectra Processor - Fun & Fixed Version ✨
================================================
Interactive Streamlit app for FTIR data processing.

Features:
- Robust ALS baseline (no more shape errors)
- Linear baseline as safe default
- Comprehensive functional group library
- Demo spectrum, batch export, peak detection
- Ko-fi support button

Run:
    pip install streamlit pandas numpy scipy plotly
    streamlit run ftir_processor_app_v3.py
"""
...
# Comprehensive functional group library
FUNCTIONAL_GROUPS = [
    ("O-H stretch (free, sharp)", 3580, 3650, "strong, sharp — alcohols, phenols"),
    ...
    ("O-H stretch (carboxylic acids)", 2500, 3300, "very broad, strong"),
    ...
    ("C-H stretch (aldehyde)", 2700, 2900, "medium, two bands"),
    ...
    ("C-F stretch", 1000, 1400, "very strong, multiple"),
    ("C-Cl stretch", 600, 800, "strong"),
    ("C-Br stretch", 500, 650, "strong"),
    ("Carbonate (CO₃²⁻)", 1400, 1450, "strong"),
    ("Sulfate (SO₄²⁻)", 1100, 1150, "strong"),
    ("Phosphate (PO₄³⁻)", 1000, 1100, "strong"),
    ("Nitrate (NO₃⁻)", 1350, 1380, "strong"),
    ("Silicate / Si-O", 900, 1100, "very strong"),
    ("Aromatic C-H oop (monosubstituted)", 690, 710, "strong"),
    ("Aromatic C-H oop (ortho)", 735, 770, "strong"),
    ("Aromatic C-H oop (meta)", 680, 710, "strong"),
    ("Aromatic C-H oop (para)", 810, 840, "strong"),
...
def apply_processing_pipeline(wn, y, params):
    ...
    if params.get("baseline", True) and params.get("baseline_method") != "None":
        ...
        if len(y) < 15:
            log.append("Skipped baseline (too short after cropping)")
        elif "ALS" in method:
            try:
                ...
                log.append("ALS failed → used Linear")
            ...
            y = y - baseline
            log.append(f"Polynomial baseline")
...
st.caption("🔬 Fixed & Improved • Stable baseline correction • Fun for researchers!")

# Fun intro
st.markdown("""
**Welcome to your FTIR playground!**  
Upload your spectra, preprocess like a pro, detect peaks, and get functional group suggestions instantly.  
Made with ❤️ for analytical chemists who love clean data.
""")
    ...
    st.markdown(
        ...
        <a href="https://ko-fi.com/jayakrishnash001" target="_blank">
            <img src="https://ko-fi.com/img/githubbutton_sm.svg" 
                 ...
                 style="width: 180px; border: 0; border-radius: 8px;" />
        </a>
        """,
        unsafe_allow_html=True
    )
    st.caption("Every coffee helps keep this tool alive! ✨")
...
with tab_upload:
    ...
    if st.session_state.spectra_data:
        ...
        st.info("Upload files or load the demo spectrum from the sidebar to get started!")
...
with tab_process:
    ...
    if not st.session_state.spectra_data:
        st.warning("Please load some spectra first (Tab 1)")
    else:
        names = list(st.session_state.spectra_data.keys())
        ...
        if selected:
            ...
            with col1:
                ...
                crop = st.checkbox("Enable cropping", True)
            ...
            with col2:
                ...
                smooth = st.checkbox("Savitzky-Golay filter", True)
                sg_window = st.slider("Window length (odd)", 5, 51, 11, 2, disabled=not smooth)
                sg_poly = st.slider("Polynomial order", 1, 5, 2, disabled=not smooth)

            with col3:
                st.markdown("**3. Baseline Correction**")
                baseline = st.checkbox("Enable baseline correction", True)
                baseline_method = st.selectbox(
                    "Method",
                    ["Linear (safe default)", "ALS (advanced)", "Polynomial (deg 3)", "None"],
                    index=0,
            ...
            if st.button("🚀 Apply Preprocessing to Selected Spectra", type="primary", use_container_width=True):
                ...
                if success:
                    st.balloons()
                    st.success(f"🎉 Successfully processed {success} spectrum/spectra!")
...
with tab_peaks:
    ...
    if not st.session_state.spectra_data:
        ...
        if not processed_names:
            st.info("Process some spectra in Tab 2 first")
        else:
            primary = st.selectbox("Choose spectrum for peak detection", processed_names)
            ...
            with c3:
                distance = st.slider("Minimum distance (points)", 5, 80, 15, 1)
            ...
            if st.button("🔎 Detect Peaks & Assign Groups", type="primary"):
                ...
                if len(peaks_idx) == 0:
                    st.warning("No peaks detected. Try lowering the height or prominence.")
                else:
...
with tab_export:
    ...
    if not processed_available:
        st.info("Process some spectra first in Tab 2")
    else:
        to_export = st.multiselect("Select spectra to export", processed_available, default=processed_available)
...
# Fun footer with Ko-fi reminder
st.divider()
st.markdown(
    """
    <div style="text-align: center; color: #666;">
        Made with ❤️ for researchers who love clean spectra.<br>
        If this tool helped you, consider buying the developer a coffee ☕
    </div>
    """,
    unsafe_allow_html=True
)